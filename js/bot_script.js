...
function initBotObjects(){
	...
	departYear[0] = initBotObject(departYear[0],year);
	...
	departYear[1] = initBotObject(departYear[1],year);
  ...
  /* added for TDP1.2+ begin */	
	...
	flexibleDatesType = initBotObject(flexibleDatesType);  
  ...
  /* added for TDP1.2+ end */		
	...
	/* added 6.05.2006 */
	isNewSearchFlow = initBotObject(isNewSearchFlow);	
...
function updateControlsState(){
	...
	if(botParams=getCookie("botparams")){
	  ...
	  /* added for TDP1.2+ begin */	
		...
		// no cookie for flexibleDatesType
...
function changeFormType(val,typeBot){
	var idFieldsFormTypeVisible="IdFieldsFormType0";
	var idFieldsFormTypeHidden="IdFieldsFormType1";
	
	if(val==1){
	 idFieldsFormTypeVisible="IdFieldsFormType1";
	 idFieldsFormTypeHidden="IdFieldsFormType0";	
	}
		
	var oFieldsFormTypeVisible=document.getElementById(idFieldsFormTypeVisible);
	var oFieldsFormTypeHidden=document.getElementById(idFieldsFormTypeHidden);	
	
	oFieldsFormTypeHidden.style.display="none";
	oFieldsFormTypeVisible.style.display="";
...
function submitBOTform(){
	...
	if (1 || tdpIsValidationRequried)
	{

		var orig=(getElementValue(form[formType.name])==0)? form[origin[0].name]: form[fakeOrigin[0].name];
		
		if(trim(getElementValue(orig))==""){
			pushError(orig,err1)
		}
		...
		var dest=(getElementValue(form[formType.name])==0)? form[destination[0].name]: form[fakeDestination[0].name];	
		
		if(trim(getElementValue(dest))==""){
			pushError(dest,err2)
		}
		...
		if(!errFlag){
			if(trim(getElementValue(orig))==trim(getElementValue(dest))){
				pushError(orig,err5)		
			}
		}
		...
		// Check date mask
		if (dateType==2) {
			if (parseDate(getElementValue(departDate[0].name),dateFormat) == null) {
				pushError(form[departDate[0].name], err12);
				errDate=true;
			}
			
			if (!errDate && getElementValue(form[tripType.name])=="RT" &&
				parseDate(getElementValue(departDate[1].name),dateFormat) == null ) {
				pushError(form[departDate[1].name], err12);
				errDate=true;
			}
		...
		if (!errDate) {	
			...
			if(!errDate){
				if(!isDateBeforeToday(getElementValue(departMonth[0].name),getElementValue(departDay[0].name),getElementValue(departYear[0].name))){
					pushError(((dateType==2)?form[departDate[0].name]:form[departMonth[0].name]),err8);
					errDate=true;
				}
				...
				if(getElementValue(form[tripType.name])=="RT"){
					if(!isDateBeforeToday(getElementValue(departMonth[1].name),getElementValue(departDay[1].name),getElementValue(departYear[1].name))){
						pushError(((dateType==2)?form[departDate[1].name]:form[departMonth[1].name]),err9);
						errDate=true;
					}				
					...
					if(!errDate){
						...
						if(depDate.getTime() + parseInt(timeCompare['t'+getElementValue(departTime[0].name)],10) > retDate.getTime() + parseInt(timeCompare['t'+getElementValue(departTime[1].name)],10)){
	...
	if(getElementValue(form[tripType.name])=="RT"){
		...
		setElementValue(form[fakeDestination[1].name],getElementValue(form[fakeOrigin[0].name]));
		setElementValue(form[fakeOrigin[1].name],getElementValue(form[fakeDestination[0].name]));
...
function isDateBeforeToday(m,d,y) {
...
try{registerJS("js/bot_script.js");}
catch(e){}